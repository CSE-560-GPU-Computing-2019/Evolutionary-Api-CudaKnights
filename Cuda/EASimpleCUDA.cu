...
__global__ void initComponent(char *genome,int lenght){
...
int i =  blockIdx.x*blockDim.x+threadIdx.x;
  ...
  int s = (int)(stop_time - start_time)%4;
		if(s<1)
			genome[i] = 'A';
		else if (s < 2)
			genome[i] = 'T';
...
{
	char *genome1;
	cudaMalloc((void**)&genome1,sizeOfGenome*sizeof(char));
	const dim3 blockSize(1024,1,1);
	const dim3 gridSize(((float) sizeOfGenome/1024),1);
	initComponent<<<gridSize,blockSize>>>(genome1,sizeOfGenome);
	cudaMemcpy(genome,genome1,sizeOfGenome*sizeof(char),cudaMemcpyDeviceToHost);
...
{	
	...
	int count=0;
	for(int i=0;i<noOfGenerations*3;i++){
		initFitness(s);
